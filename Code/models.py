...
class generic_wlnn_regression(torch.nn.Module):
    def __init__(
        self,
        pool_func: torch.nn.Module,
        encoding: torch.nn.Module,
...
class generic_gnn_regression(torch.nn.Module):
    def __init__(
        self,
        gnn: torch.nn.Module,
        pool_func: torch.nn.Module,
...
def create_model(
    ...
    if model_name.startswith("1WL+NN:"):
        ...
        if "Embedding" in model_name:
            assert (
                encoding_kwargs["embedding_dim"] > 0
                and encoding_kwargs["max_node_feature"] > 0
            ), "Key 'embedding_dim' and 'max_node_feature' must be defined in encoding_kwargs as a postive integer greater zero."
        ...
        if "Sum" in model_name:
            ...
            assert (
                encoding_kwargs["processing_steps"] > 0
            ), "Key 'processing_steps' must be defined in encoding_kwargs as a postive integer greater zero."

            pool_func = torch_geometric.nn.aggr.Set2Set(
                in_channels=mlp_input_channels, **encoding_kwargs
            )
        ...
        if is_classification:
            model = generic_wlnn(encoding=encoding, mlp=mlp, pool_func=pool_func)
        else:
            model = generic_wlnn_regression(
                encoding=encoding, mlp=mlp, pool_func=pool_func
            )

    # Check if the model is a GIN model
    elif (
        model_name.startswith("GIN")
        or model_name.startswith("GAT")
        or model_name.startswith("GCN")
    ):
        # Construct the GNN
        ...
        # Construct the model
        if is_classification:
            model = generic_gnn(gnn=gnn, mlp=mlp, pool_func=pool_func)
        else:
            model = generic_gnn_regression(gnn=gnn, mlp=mlp, pool_func=pool_func)

...
\begin{theorem}
    Let $\cC$ be a collection of permutation-invariant functions that is \textbf{\wldisc}, than $\cC$ is also \textbf{\gapp}.
\end{theorem}
...
\begin{lemma}[\wlnn Permuation Invariance]
...
\begin{lemma}[Composition Lemma]\label[lemma]{lem:composition_lemma}
    Let $\cC$ be a collection of functions computed by $\wlnn$. Further, $h_1, \dots h_n \in \cC$ and $\text{MLP}$ a multilayer perceptron, than the function $\cA$ composed of $\cA(G^*) := \text{MLP}(h_1(G^*), \ldots, h_n(G^*))$ is also computable by \wlnn.
\end{lemma}
\begin{proof}
    Assume the above and let $f_{\text{enc}}$ be the encoding functions used by $h_1, \dots h_n$. Further, let $\text{MLP}_1, \ldots, \text{MLP}_n$ be the multilayer perceptrons the functions $h_1, \dots h_n$ are composed of respectively. The idea of this proof is, that we construct a new MLP that first duplicates the encoding vector $n$ times, simulates $\text{MLP}_1, \ldots, \text{MLP}_n$ each on a copy of the encoding vector in parrallel, and then use the concatetated outut of all of them as input for the new final MLP. See \autoref{fig:proof_idea_parallelism} for a sketch of the proof idea. A complete proof can be found in the Appendix, as this proof is very technical and not that interesting.
\end{proof}
...
\begin{lemma}[My Lemma]\label[lemma]{lem:encoding-indicator-func1}
    Let $\cC$ be a collection of permuation-invariant functions from $\xnn$ to $\Rb$ computed by \wlnn that is \wldisc. Then for all $G \in \xnn$, there exists a function $h_G$ computable by \wlnn such that for all $G^* \in \xnn: h_G(G^*) = 0$ if and only if $G \wliso G^*$.
\end{lemma}

...
This section is the main part of our theoretical investigation of the two frameworks. We will present 4 intriguing theorems, which will be proven separately afterwards. These results will form the basis for the empirical part that follows. The first two theorems will establish an equivalence between the two frameworks when the input set of graphs is finite. The last two theorems will go one step further and establish a connection for continuous functions computed by $\wlnn$ and GNNs and prove a somewhat weaker connection between them.\todo{add that graph wise classification implies node classification!!!}
...
Since we only wanted to show the expressive power of $\wldisc$ and made the major assumption of the existence of a continuous $\wl$ algorithm, we have included the proofs of each theorem in the Appendix in \autoref{app:gnn_in_wlnn}.
...
\begin{proof}
    ...
    \begin{align*}
        C := \{ \cB_c: \cX \rightarrow \Rb, \ G \mapsto \mlp_\text{id} \circ f_c \MSopen \wl(G)(c) \mid v \in V(G) \MSclose \mid c \in \Nb\},
    \end{align*}

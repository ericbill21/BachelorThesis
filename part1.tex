...
\begin{theorem}[General Case: ``$\text{GNN} \subseteq \wlnn$'']\label{theorem:1wl_in_gnn_approximating}
    Let $\cC$ be a collection of continuous functions from $\cX$ to $\Rb$ that is $\wldisc$, then $\cC$ is also GNN-Approximating.
\end{theorem}
...
\begin{lemma}
    Let $\cC$ be a collection of continuous functions from $\cX$ to $\Rb$. If $\cC$ is 1-\!WL-Discriminating, then for any chosen graph $G^* \in \cX$ and any $\epsilon \in \Rb$ with $\epsilon > $, the function $h_{G^*}$ from $\cX$ to $\Rb$ with the following properties:
    \begin{enumerate}
        \item For all $G \in \cX:  h_{G^*}(G) \geq 0$.
        \item For all $G \in \cX$ with $G \wliso G^*: h_{G^*}(G)=0$.
...
One can interpret this function $h_{G^*}$ as a kind of loss function that measures the similarity between two graphs. It yields no loss for the input $G$ if $G$ is indistinguishable from $G^*$ by the algorithm $\wl$ ($G \wliso G^*$), only a small loss if $G$ is close to $G^*$ (the loss is upper bounded by $\delta_{G^*}$), and a large loss otherwise.
...
\begin{proof}
   ...
   Furthermore, by assumption $h_{G, G^*}$ is continuous, $A_{G, G^*}$ is an open set. For every $G \in \cX$ with $G \wliso G^*$ we define $A_{G, G^*}$ as follows:
    ...
    \begin{equation}\label{eq:continous_proof4}
    ...
    \begin{align}
        ...
        &= \max(h_{G_0, G^*}(\cdot) - h_{G_0, G^*}(G^*)) + \max(h_{G_0, G^*}(G^*) - h_{G_0, G^*}(\cdot))\label{eq:continous_proof5}
    \end{align}
    ...
    \begin{enumerate}
        ...
        \end{align}
        To understand these inequalities, it helps to visualize them, see therefore \autoref{fig:proof_support}. We will try to give an explanation now, using the colored distances depicted in \autoref{fig:proof_support}. In \autoref{eq:continous_proof1}, we use the fact that the red distance is always greater than the green minus the blue distance. Further, using \autoref{eq:continous_proof3}, we know that the blue distance is always smaller than half of than the green distance. Using this fact, it is easy to see that in \autoref{eq:continous_proof2} the green minus the blue distance is always greater than or equal than the half of the green distance.
    ...
    As a final note, the construction of $h_{G_0, G^*}(\cdot)$ shown in \autoref{eq:continous_proof4} can be encoded by a single layer multilayer perceptron, because the overall sum is finite as $\cX_0$ is finite, and each summand, defined as in \autoref{eq:continous_proof4}, can be encoded by replacing the $\max$ operator by the non-linear activation function ReLU.

\end{proof}

\begin{lemma}
    Let $\cC$ be a collection of continuous functions from $\cX$ to $\Rb$. If for every $\epsilon > 0$ exists $h_{G^*} \in \cX$ for any graph $G^* \in \cX$ with the following properties:
    \begin{enumerate}
        \item For all $G \in \cX:  h_{G^*}(G) \geq 0$.
        \item For all $G \in \cX$ with $G \wliso G^*: h_{G^*}(G)=0$.
        \item There exists a constant $\delta_{G^*} > 0$, such that for all $G \in \cX$ with $h_{G^*}(G) < \delta_{G^*}$ there exists a graph $G' \in \cX/\!{\wliso}(G)$ in the equivalence class of $G$ such that $\| G' - G^* \|_2 < \epsilon$.
    \end{enumerate}
    Then $\cC$ is also GNN-Approximating.
\end{lemma}
...
\begin{proof}
    ...
    For any $G \in \cX$, we define the set $B_{G, a} := \{ G' \in \cX \mid h_G(G') \in [0, a) \}$, as the set of graphs that are mapped into the open interval $[0, a)$ by $h_G$. Then, by definition of $h_G$, there exists a constant $\delta_G$ such that:
    \begin{equation*}
        B_{G, \delta_G} \subseteq \bigcup_{G'\in \cX/\!{\wliso}(G)} \{ G'' \in \cX \mid \| G'' - G' \|_2 < r \}.
    \end{equation*}
    Since $h_G$ is continuous by definition, $B_{G, \delta_G}$ is an open set. Hence, $\{ B_{G, a} \}_{G \in \cX}$ is an open cover of $\cX$, and further as $\cX$ is compact, there exists a finite subset $G_0 \subseteq \cX$ such that $\{ B_{G_0, a} \}_{G_0 \in \cX_0}$ is also a cover of $\cX$. For each $G_0 \in \cX_0$, we construct the function $\varphi_{G_0}$ on $\cX$ as follows:
    \begin{equation*}
        \varphi_{G_0} (\cdot) := \max(\delta_{G_0} -  h_{G_0}(\cdot), \ 0).
    \end{equation*}
    The function has two important properties, for once it is non-negative and for any $G \in \cX: \varphi_{G_0}(G) > 0$, if and only if $G \in B_{G_0, \delta_{G_0}}$, thereby acting as a sort of weak indicator function. Building up on this property, we construct for each $G_0 \in \cX_0$ the function $\psi_{G_0}$ on $\cX$ as follows:
    \begin{equation*}
        \psi_{G_0}(\cdot) := \frac{\varphi_{G_0(\cdot)}}{\sum_{G' \in G_0} \varphi_{G'}(\cdot)},
    \end{equation*}
    which is well-defined, because $\{ B_{G_0, a} \}_{G_0 \in \cX_0}$ is a cover of $\cX$, such that we can conclude that for any input graph $G$ on $\psi_{G_0}(\cdot)$ there exists a $B_{G_0, \delta_{G_0}}$ with $G \in B_{G_0, \delta_{G_0}}$ implying $\varphi_{G_0}(G) > 0$, making the denominator not $0$. The function $\psi_{G_0}$ has two important properties, for once it is non-negative, because $\varphi_{G}$ for all $G \in \cX$ is non-negative, and for any $G \in \cX: \psi_{G_0}(G) > 0$, if and only if $G \in B_{G_0, \delta_{G_0}}$.

    Further, we can observe that the set of functions $\{ \psi_{G_0}\}_{G_0 \in \cX_0}$ is a partition of unity on $\cX$ with respect to the open cover $\{ B_{G_0, a} \}_{G_0 \in \cX_0}$, because:
    \begin{enumerate}
        \item For any $G \in \cX$ the set of functions mapping $G$ not to $0$ is finite, as the set of all functions $\{ \psi_{G_0}\}_{G_0 \in \cX_0}$ is finite.
    ...
    \end{equation*}
    Recall the property from the beginning of the proof that for any $G \in \cX$ if $G \in B_{G_0, \delta{G_0}}$, then there exists a $G' \in \cX/\!{\wliso}(G)$ with $\| G' - G_0 \|_2 < r$, which implies that $|\cA(G) - \cA(G_0)| < \epsilon$. With this, we can construct a function $\hat{\cA}$ on $\cX$ that approximates $\cA$ within accuracy $\epsilon$:
    \begin{equation*}
    ...
    \begin{align*}
        ...
        &< \sum_{G_0 \in \cX_0} \epsilon \cdot \psi_{G_0}(G)\\

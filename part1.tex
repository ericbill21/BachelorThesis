...
\begin{definition}[Graph Neural Network]\label{def:gnn}
    ...
    \begin{align*}
        &\mathcal{A}: \mathcal{G} \rightarrow \mathcal{Y}, \ G \mapsto G' := (V(G), \ E(G), \ f^{(K)}),
    \end{align*}
...
In this section we will shortly prove the converse direction. However, the proof is not as interesting as the previous one, as it is mainly just a consequence of \autoref{def:gnn} of GNNs. \todo{!}
...
\begin{align*}
C := \{ \textsf{Readout}_c(\MSopen f^{(n)}(v) \mid v \in V(G) \MSclose) \mid c \in \Nb \},
\end{align*}
where $\textsf{Readout}_c$ is the \textsf{READOUT} function that returns the number of nodes colored $c$ in the coloring of $f^{(n)}$.
\end{proof}

\begin{lemma}
    Let $C$ be a collection of function computable by GNNs, $\cA_1, \dots , \cA_n \in C$ and $\textsf{Readout}$ as suitable \textsf{READOUT} function for GNNs, then $\hat{\cA} := \textsf{Readout}(\cA_1, \cA_n)$
\end{lemma}

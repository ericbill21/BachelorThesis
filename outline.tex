...
In this section, we will briefly introduce the foundation of our research by explaining the origins of each method, mentioning recent important advances, and providing a brief overview of the connection between them.
...
From the work of \cite{Gil+2017}, as well as \cite{Sca+2009}, the so-called message-passing architecture emerged for GNNs, which are also sometimes referred to as message-passing neural networks (MPNNs). This can be understood as a framework that never changes its input graph structurally and only modifies the node's features in each layer simultaneously. In more detail, in each layer, a GNN based on the message-passing architecture computes for each node a new feature, based on its current feature and the features of its neighbors using for example a feedforward neural network. Later in \autoref{sec:GNN Defintion}, we will give a more formal definition of this architecture.

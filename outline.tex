...
If our empirical results support this assumption, we will further investigate what specific phenomena cause this difference, for which we will develop more test cases. 

As an example of how we will show the existence of a local bias to local structures in GNNs, we will create an artificial graph dataset where each graph is composed of one node marked as source, one node marked as target, and the rest marked as neutral. The task is to classify whether there exists a path between the source node and the target node. We will ensure that the minimum distance between these nodes is never greater than the total number of layers of the tested GNN, so that we can ensure that information can be exchanged between the source and target nodes. The intuition here is that a \wlnn model should achieve near-perfect performance because it uses only structural information as basis of its decision, whereas a GNN model suffers from the over-squashing effect, where the farther apart two nodes are, the less information is exchanged between them, making it challenging for the model to correctly classify if there exists a path between the source and target nodes.

We will develop more sophisticated methods and approaches as the work progresses, as we cannot speculate on the outcome for now.

...
The empirical part of this thesis is organized into three chapters. Firstly, we cover the methodology used for conducting our experiments. Afterward, in \cref{sec:emprical_results}, we explore the results of the experiments and analyze various properties of both model types in detail. Finally, \cref{sec:discussion} concludes the thesis with a comprehensive discussion, highlighting insights, identifying issues, and suggesting future research directions.
...
\FloatBarrier
\section{Approximating \wl Coloring: Investigating \gnn Node Representations}
...
One significant advantage lies in the ease of deployment. Configuring a \wlnn model involves only selecting a suitable encoding function and setting the size of its multilayer perceptron. Additionally, the colors computed by the \wl algorithm remain fixed and only need to be calculated once during model initialization. Consequently, the training process of a \wlnn model mainly revolves around optimizing the multilayer perceptron, making it notably faster than training \gnn models. For instance, our experiments demonstrated that training \gin:\textsf{Sum} on the \zinc dataset took nearly 13 hours, while our best \wlnn model completed training in under 3 hours.

However, despite these advantages, it is crucial to acknowledge that \wlnn models are restricted in their real-world applicability. The requirement to initialize the \wl algorithm with unique colors for all substructures the model will encounter before training makes the \wlnn framework more suitable for research purposes and tasks with a known set of input graphs. In contrast, \gnns exhibit greater versatility, proving effective on any graph and thus better suited for real-world applications.

In conclusion, \wlnn models offer valuable insights into \gnn behavior and performance evaluation for datasets. Their advantages make them well-suited for research and understanding the theoretical underpinnings of \gnns. 
...
\section{Conclusion}
In conclusion, this thesis has introduced the novel \wlnn framework. We have proved the theoretical equivalence to \gnns in terms of computability and demonstrated how well it is suitable for investigating \gnns. Leveraging this framework as a tool for understanding \gnns, we learned that \gnn models are more efficient in generalizing and encoding an efficient coloring of their nodes, such that they encode just as much structural information as required for solving the respective tasks.

As the field of graph representation learning continues to evolve, future research in the areas we highlighted can pave the way for exciting discoveries and advancements in this domain.
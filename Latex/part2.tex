...
\end{description}
To address these questions, this part is structured into three sections. First, we explain the methodology for conducting our experiments, covering model and dataset selection, the testing procedure, and hyperparameter choices. Subsequently, in \cref{sec:emprical_results}, we comprehensively explore the results obtained from the experiments and examine various properties of both model types in detail. Finally, \cref{sec:discussion} concludes this thesis with a discussion that consolidates all insights, answers the research questions, highlights any identified issues, and provides recommendations for future research directions.
...
Our primary consideration for the \wlnn models revolves around choosing an encoding function, as all other components are fixed. We utilize encoding functions with two main components: an optional preprocessing step of the color computed by the \wl algorithm and a basic pooling function to convert the color histogram into a fixed-size vector.
...
\subsubsection{Implementation Details and Result Reproducibility}
...
\section{Empirical Results}\label{sec:emprical_results}
...
\begin{table}[bh]
	...
	\resizebox{0.975\textwidth}{!}{ 	\renewcommand{\arraystretch}{1.05}
		\begin{tabular}{@{}c <{\enspace}@{}lcccc@{}}	\toprule
			...
			& \textsf{\gineeps} & 0.103 {\scriptsize $\pm  0.001$} -2.956 {\scriptsize $\pm  0.029$}\tablefootnote[8]{The results of \gineeps on \alchemy and \zinc are adopted from \cite{Mor+2020}.} & 0.180 {\scriptsize $\pm  0.006$} -1.958  {\scriptsize $\pm  0.047$}\tablefootnote[9]{\label{foot:Mor22}The results of \gineeps on \alchemyten is adopted from \cite{Morris2022}.} & 0.084 {\scriptsize $\pm  0.004$}\textsuperscript{8} & -
			\\
			\cmidrule{2-6}
...
\FloatBarrier
\subsection{Fixed \wl Iterations: Tradeoff between Expressiveness and Generality}\label{sec:tradeoff_expressiveness}
...
\begin{figure}[htb]
	...
	\end{subfigure}
	\caption{Overview of the performance achieved by all tested \wlnn configurations grouped by the fixed number of \wl iterations for each dataset. The y-axis uses different metrics based on dataset type: accuracy in percent for classification datasets and absolute mean error for regression datasets.}
...
Interpreting the results of the \textsc{Zinc} dataset is more complex. While the results on the smaller subset of the dataset, \textsc{Zinc10K}, clearly indicate that fewer \wl iterations yield better results, the results on the full \textsc{Zinc} dataset suggest the opposite. This discrepancy could be attributed to the limited number of runs conducted on the entire dataset, which were restricted due to scalability and time constraints. Therefore, these results should be interpreted with caution.
...
\begin{figure}[htb]
	...
	\end{subfigure}
	\caption{Impact of data clustering on \textsf{$k$-NN} classifier performance for two binary classification datasets with 1000 two-dimensional data points. One dataset exhibits good clustering, while the other does not. The \textsf{$k$-NN} classifier accuracy is plotted for various $k$ values.}
	\label{fig:knn_sketch}
\end{figure}\todo{Caption is awful!!}

Before analyzing \cref{fig:pool_knn}, we explain why the existence of such a range indicates good clustering. Consider the two artificial scenarios in \cref{fig:knn_sketch}, where we construct two balanced datasets with 1000 two-dimensional data points for a binary classification task. The dataset in \cref{fig:knn_sketch_good} clusters well, meaning it is easy to determine which region in the feature space corresponds to each class, while the other dataset in \cref{fig:knn_sketch_bad} clusters poorly. By training a \textsf{$k$-NN} classifier on both datasets individually for various $k$, we observe the following: If the data clusters well, the accuracy achieved by the \textsf{$k$-NN} classifier will be consistently high and not fluctuate significantly for different values of $k$. Conversely, accuracies around \perc{50} with strong fluctuations indicate poor clustering.
...
In the case of the \enzymes dataset, although there is exist no such a plateau, the accuracy achievable on the graph representations is for both types of models very similar, such that the lack of indication for clustering can probably be attributed to the dataset itself.
...
\FloatBarrier
\subsection{Combining the \wl algorithm with \gnns}\label{sec:wl_with_gnn}
...
In conclusion, the impact of the size of a dataset on the performance of \wlnn models must be investigated further. In detail, future research needs to investigate datasets not only of large size but also datasets that consist of large graphs, such that the number of unique colors is already relatively high even after a single iteration of the \wl algorithm. A good example of such a dataset is the \reddit dataset, where the number of unique colors after a single iteration of the \wl algorithm is 566 due to the high number of nodes on average per graph. For this dataset, a performance gap between \wlnn and \gnn models can be observed, with the best \gnn model achieving up to an \perc{8} higher accuracy than the best \wlnn model. However, it is also essential to acknowledge that the reasons for this performance gap are not solely attributable to the dataset's size, as other factors are at play, as discussed in the preceding subsections.

\section{Discusssion}\label{sec:discussion}
This concluding section will discuss and summarize the key findings and conclusions from our empirical investigation by addressing our initial research questions. Moreover, we examine the use case of the \wlnn framework as a valuable research tool and outline potential areas for future research. Finally, we conclude the thesis with some final remarks.

\subsection{Insights into the Representations learned by \gnns}
This section concludes our empirical investigation and summarizes our findings by answering our initial research questions.

\paragraph{Q1} Can \wlnn models achieve comparable performance to \gnn models empirically?

\vspace{-15pt}

\paragraph{A1} Yes, the results on the classification datasets indicate that \wlnn models achieved comparable performance and even outperformed \gnn models on 4 out of 6 datasets. However, regression datasets showed a performance discrepancy between \gnn and \wlnn models, suggesting that \wlnn models might be more suitable for classification tasks than regression tasks due to the discrete nature of the \wl algorithm.

\paragraph{Q2} Are there observable differences in the learning behavior between \wlnn and \gnn models?

\vspace{-15pt}

\paragraph{A2} Yes, \wlnn models tend to exhibit more pronounced overfitting compared to \gnn models, primarily due to the expressive nature of the \wl algorithm and the detailed colorings it produces. Ensuring good generality of \wlnn models requires limiting the expressiveness of the \wl algorithm in most applications. Furthermore, we observed that while \wl assigns the majority of nodes in a graph a unique color, \gnn computes node representations to ensure the euclidean distance between nodes is almost uniformly distributed between the minimum and maximum distance.
...
\paragraph{A3} In the majority of cases, the \wlnn models do not fully utilize the expressiveness of the \wl algorithm to achieve comparable performance. Most best-performing \wlnn models utilize only a single iteration of the \wl algorithm, effectively equivalent to a node degree encoding. Interestingly, \gnns also mainly rely on this information, computing node representations similar to those produced by a single iteration of the \wl algorithm. However, there are exceptions where utilizing an increased number of \wl iterations for a \wlnn model improves its performance, demonstrating the potential of the \wl algorithm's expressiveness. In these exceptional cases, \gnns also show better approximation capability of the coloring computed by the \wl algorithm. This observation leads us to conclude that \gnn models strike a balance between efficient encoding and required expressiveness depending on the application.
...
\subsection{The Use of the \wlnn Framework beyond the Scope of this Work}
This work has demonstrated numerous similarities between the \wlnn and \gnn frameworks. In particular, we established the theoretical equivalence in terms of computability and observed many similarities we investigated in detail in the previous section. As a result, the \wlnn framework proves to be a valuable tool for studying \gnn models and datasets. Moreover, it offers certain advantages over \gnns.

Such an advantage is the ease of deployment. Setting up a \wlnn model requires only selecting an appropriate encoding function and configuring the size of its multilayer perceptron. 

Additionally, since the colors computed by the \wl algorithm do not change and therefore only need to be computed once, during the initialization of the model. The training phase of a \wlnn model mainly involves optimizing the multilayer perceptron. As a result, training \wlnn models can be significantly faster than \gnn models. For instance, in our experiments, training \gin:\textsf{Sum} on the \zinc dataset required almost 13 hours, whereas our best \wlnn model completed training in less than 3 hours.

However, despite these advantages, \wlnn models have limitations that restrict their real-world applicability. The need to initialize the \wl algorithm with unique colors for all substructures before training makes the \wlnn framework more suitable for research purposes and tasks with a known set of input graphs. In contrast, \gnns exhibit greater versatility, being effective on any graph and thus better suited for real-world applications.

In conclusion, \wlnn models provide valuable insights into \gnn behavior and performance evaluation. However, due to their limitations, they are limited to lab settings and not recommended for real-world use.
...
A primary consideration is the time constraints that impacted the extent of experimentation in this study, specifically in Sections 2 and 3.

The impact of the \wl algorithm preprocessing on \gnn models, as discussed in \cref{sec:wl_with_gnn}, warrants further investigation. While initial results did not show significant improvements, they at least demonstrated comparable performance to \gnns. Investigating the reasons behind this phenomenon and conducting additional hyperparameter optimization runs for \wl:\gnn models will shed light on the potential advantages and limitations of incorporating \wl preprocessing into \gnn models.

Similarly, the discrepancies in performance observed on the regression datasets, as discussed in \cref{sec:size_impact}, raise questions about whether this discrepancy is inherent to the nature of the regression task or influenced by the dataset size. To address this, further experiments on smaller-scale regression datasets and larger-scale classification datasets, such as \textsc{MalNet} curated by \cite{Freitas2022}, would provide valuable insights into the scalability and generalizability of both \wlnn and \gnn models.

Additionally, extending the study to investigate more specialized models within the \wlnn framework would be an intriguing research direction. The empirical similarities observed between \wlnn and \gnn models open up possibilities for exploring highly specialized \wlnn models and their unique characteristics. This research direction could focus on understanding the specific functionalities and advantages of these specialized models, using \wlnn models as a versatile tool for investigating different aspects of \gnn behavior.

In summary, while this study has provided valuable insights into the behavior and capabilities of \wlnn and \gnn models, there are still numerous unexplored avenues for future research. Conducting more extensive experiments, exploring the potential of \wl algorithm preprocessing, and investigating specialized \wlnn models will contribute to a deeper understanding of these frameworks and their applications.

\subsection{Conclusion}
In conclusion, our empirical investigation has demonstrated the theoretical equivalence and many empirical similarities between \wlnn and \gnn models. \wlnn models offer valuable insights into understanding \gnn behavior and capabilities and serve as a powerful tool for investigating \gnn models and datasets. However, their limitations restrict their applicability to real-world scenarios, making \gnn models more suitable for practical applications. As the field of graph representation learning continues to evolve, future research in the areas we highlighted can pave the way for exciting discoveries and advancements in this domain.
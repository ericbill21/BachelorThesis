...
\begin{table}[!htb]
	...
	\newcommand{\seq}[2]{$(#1, #2)$\text{-}\textsf{SpeqNet}\xspace}
		\begin{tabular}{@{}lcccc@{}}	\toprule
			...
			\\
			\textsf{GIN:Mean} & 0.643 {\scriptsize $\pm 0.014$} -0.505 {\scriptsize $\pm 0.021$} & 0.314 {\scriptsize $\pm 0.004$} -1.469 {\scriptsize $\pm 0.044$} & & 0.341 {\scriptsize $\pm 0.037$}
			\\
			\midrule
...
\begin{table}[!htb]
    \resizebox{.975\textwidth}{!}{ 	\renewcommand{\arraystretch}{1.05}
		\begin{tabular}{@{}c <{\enspace}@{}lcccccc@{}}	\toprule
			...
			\toprule
			\multirow{1}{*}{}
			& Standard \wl & 34.2 {\scriptsize $\pm 6.7$} $(3.0)$	& 67.0 {\scriptsize $\pm 4.3$} $(1.2)$ & 76.3 {\scriptsize $\pm 9.4$} $(4.2)$ & $(3.9)$ & 71.4 {\scriptsize $\pm 4.9$} $(3.0)$ & 70.9 {\scriptsize $\pm 3.8$} $(4.1)$
			\\
			& Parametrized \wl & 48.3 {\scriptsize $\pm 8.1$} $(1)$ & 72.4 {\scriptsize $\pm 4.1$} $(1)$ & 85.1 {\scriptsize $\pm 8.6$} $(1)$ & 83.6 {\scriptsize $\pm 2.2$} $(3)$ & 75.2 {\scriptsize $\pm 3.9$} $(1)$ & 78.4 {\scriptsize $\pm 2.7$} $(1)$
			\\
			\bottomrule
		\end{tabular}}
...
A problem with the \wlnn models is that the \wl alogirhtm in particular is too expresives and this has an direct impact on the learning performance.

In order to grasp how expressive it is, we calculated for each dataset the ratio of unique colors compared to total amount of colors. For example, the dataset Mutag consists of of 188 graphs in total. If we sum up the number of nodes of each graph, we end up with 3371 nodes in total. This is the upperbound of the total amount of colors the wl alogirghtm can apply use for calculating the colorings. If we now apply the wl alogirhm with a fixed number of wl iterations of 1 on Mutag, we end up with a coloring of each graph. If we now count the number of unique colors used we end up with 231 unique colors, which in comparison to the number of total nodes if very considerable, such that the ratio of used colors for the first iteration is \perc{11.8}.

...
\begin{table}[!htb]
    \centering
    \caption{Overview of the construction of the \textit{message-passing} layers and their respective learnable parameters by popular \gnn model. This format is inspired by \cite{Nikolentzos2023}.}
    \label{tab:models}
    ...
    \begin{tabular}{ c| c |c}
        ...
        \hline
        GAT & \Centerstack{
            $f^{(t)}(v) = \sigma \biggl( \nnsum_{u \in \cN(v)} \alpha_{vu} \cdot W^{(t)} \cdot f^{(t-1)}(u) \biggr)$\\
            \vspace*{5pt}\\
            $\text{with}\quad \alpha_{vu} = \frac{\exp\left(\text{LeakyReLU}\left(\vec{ a}^T \cdot  \textsf{concat}[{ W^{(t)}}f^{(t-1)}(v), \ { W^{(t)}}f^{(t-1)}(u)]\right)\right)}{\sum_{k\in\mathcal{N}(v)} \exp\left(\text{LeakyReLU}\left(\vec{ a}^T \cdot \textsf{concat}[{ W^{(t)}}f^{(t-1)}(v), \ { W^{(t)}}f^{(t-1)}(k)]\right)\right)} $} & $\vec{a}, W^{(t)}$\\
        \hline
        GCN & $f^{(t)}(v) = \text{ReLU}\biggl( \nnsum_{u \in \cN(v) \cup \{v\}} \frac{W^{(t)}}{\sqrt{(1 + d(v))\cdot (1 + d(u))}} f^{(t-1)}(u)\biggr)$ & $W^{(t)}$ \\
        \hline
        GIN & $f^{(t)}(v) = \mlp^{(t)} \biggl( \bigl( 1 + \epsilon^{(t)} \bigr) \cdot f^{(t-1)}(v) + \nnsum_{u \in \cN(v)} f^{(t-1)}(u)\biggr)$ & $\epsilon^{(t)}, \mlp^{(t)}$\\
    \end{tabular}}
\end{table}

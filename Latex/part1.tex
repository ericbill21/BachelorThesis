...
\begin{lemma}[\wlnn Composition]\label[lemma]{lem:composition_lemma}
    Let $\cC$ be a collection of functions computable by \wlnn. Further, let $\cB_1, \dots \cB_n \in \cC$ and $\mlp^\bullet$ be a multilayer perceptron operating from $\Rb^n$ to $\Rb$, then the function $\hat{\cB}$ composed  as follows:
    \begin{equation*}
        \hat{\cB} : \cX \rightarrow \Rb,\ G \mapsto \mlp^\bullet \Bigg(
        \begin{bmatrix}
            \cB_1(G)\\
             \vdots \\
            \cB_n(G)
